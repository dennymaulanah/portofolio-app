<?= view('admin/layouts/header', ['activePage' => 'proyek', 'pageTitle' => 'Tambah Proyek']) ?>
<?= view('admin/layouts/sidebar', ['activePage' => 'proyek', 'pageTitle' => 'Tambah Proyek']) ?>
...
<main class="ml-0 md:ml-64 mt-16 p-4 md:p-8 lg:p-12 relative z-10 space-y-8 md:space-y-12">
    ...
    <section class="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
            ...
            <h2 class="font-headline-lg text-headline-lg text-on-surface">Tambah Proyek Baru</h2>
            <p class="font-body-lg text-body-lg text-on-surface-variant mt-2 max-w-2xl">Buat proyek baru untuk ditampilkan di portofolio Anda.</p>
        </div>
    </section>
    ...
    <!-- Form -->
    <form action="<?= base_url('/admin/proyek/simpan') ?>" method="POST" enctype="multipart/form-data" id="formTambahProyek">
        <?= csrf_field() ?>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="glass-card p-6 md:p-8 rounded-2xl space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select
                            ...
                            <option value="Web Dev" <?= old('kategori') === 'Web Dev' ? 'selected' : '' ?>>Web Dev</option>
                            <option value="Mobile" <?= old('kategori') === 'Mobile' ? 'selected' : '' ?>>Mobile</option>
                            <option value="UI/UX" <?= old('kategori') === 'UI/UX' ? 'selected' : '' ?>>UI/UX</option>
                            <option value="Desktop" <?= old('kategori') === 'Desktop' ? 'selected' : '' ?>>Desktop</option>
                            <option value="API" <?= old('kategori') === 'API' ? 'selected' : '' ?>>API</option>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea
                            ...
                            class="form-input w-full bg-surface-container-lowest border border-white/10 rounded-xl px-4 py-3 text-sm text-on-surface placeholder:text-on-surface-variant/40 focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/30 transition-all resize-none"><?= old('deskripsi') ?></textarea>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="hidden" id="teknologi" name="teknologi" value="<?= old('teknologi') ?>">
                ...
                <div class="glass-card p-6 md:p-8 rounded-2xl space-y-6">
                    <div class="flex items-center gap-3 mb-2">
                        <div class="w-10 h-10 rounded-xl bg-secondary/10 border border-secondary/20 flex items-center justify-center">
                            <span class="material-symbols-outlined text-secondary text-xl">photo_library</span>
                        </div>
                        <div>
                            <h3 class="font-headline-md text-sm font-bold text-on-surface">Foto Proyek</h3>
                            <p class="text-xs text-on-surface-variant/60">Upload satu atau beberapa gambar</p>
                        </div>
                    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="glass-card p-6 rounded-2xl space-y-5 lg:sticky lg:top-24">
                    ...
                    <!-- Status Preview -->
                    <div id="statusPreview" class="p-3 rounded-xl bg-amber-500/5 border border-amber-500/20 flex items-center gap-3">
                        <span id="statusDot" class="w-2.5 h-2.5 rounded-full bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]"></span>
                        <span id="statusText" class="text-xs font-semibold text-amber-400">Draft — Belum dipublikasikan</span>
                    </div>
                    ...
                    <!-- Tips Card -->
                    <div class="glass-card p-6 rounded-2xl border-primary/10 bg-primary/[0.02]">
                        <div class="flex items-start gap-3">
                            <span class="material-symbols-outlined text-primary/50 text-lg mt-0.5">lightbulb</span>
                            <div class="space-y-2">
                                <p class="text-xs font-bold text-primary/70">Tips</p>
                                <ul class="text-[11px] text-on-surface-variant/50 space-y-1.5 leading-relaxed">
                                    ...
                                    <li class="flex items-start gap-1.5">
                                        <span class="text-primary/40 mt-0.5">•</span>
                                        Upload minimal 1 screenshot proyek
                                    </li>
                                    <li class="flex items-start gap-1.5">
                                        <span class="text-primary/40 mt-0.5">•</span>
                                        Gunakan status Draft untuk menyimpan dulu
                                    </li>
                                </ul>
                            </div>
                        </div>
...
`;
    ...
    function handleFileSelect(files) {
        ...
        if (selectedFiles.length + validImages.length > 10) {
            showToast('Maksimal 10 foto yang diperbolehkan per proyek.');
            return;
        }
...
`;
    ...
    tagInput.addEventListener('input', function() {
        ...
        const container = tagSuggestions.querySelector('div');
